...
-- EI START
	...
	RESPOND_TO_ENEMY_SOUND = function(self, entity, range, distance)
--		if (distance <= range) then
			Log(entity:GetName().." combat sound. Switching to SEEK");
			AIBehavior.DEFAULT:ABORT_ALL_ACTIONS(entity, entity);
			entity:CancelSubpipe();
			AI.Signal(SIGNALFILTER_SENDER,1, "TO_SEEK", entity.id);

			--Log(entity:GetName().." combat sound. Switching to PURSUE. ("..distance..")");
			--AIBehavior.DEFAULT:ABORT_ALL_ACTIONS(entity, entity);
			--entity:CancelSubpipe();
			--AI.Signal(SIGNALFILTER_SENDER,1, "TO_PURSUE", entity.id);
--		else
--			Log(entity:GetName().." combat sound. Too far, so ignoring. ("..distance..")");
--		end
	end,

	RESPOND_TO_ENEMY_SEEN = function(self, entity)
		Log(entity:GetName().." enemy seen. Switching to PURSUE.");
		AIBehavior.DEFAULT:ABORT_ALL_ACTIONS(entity, entity);
		entity:CancelSubpipe();
		--AI.Signal(SIGNALFILTER_ANYONEINCOMM, 1, "TO_PURSUE", entity.id);
		AI.Signal(SIGNALFILTER_SENDER, 1, "TO_PURSUE", entity.id);
	end,

	AGGRO_AND_CALL_FOR_HELP = function(self, entity)
		Log(entity:GetName().." aggro and calling for help.");
		AIBehavior.DEFAULT:ABORT_ALL_ACTIONS(entity);
		entity:CancelSubpipe();
		--AI.Signal(SIGNALFILTER_ANYONEINCOMM, 1, "TO_PURSUE", entity.id);
		AI.Signal(SIGNALFILTER_SENDER, 1, "TO_SEEK", entity.id);
	end,
