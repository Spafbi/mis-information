...
function SinglePlayer:EquipActor(actor)
	...
	if (actor.actor and actor.actor:IsMyPlayer()) then -- EI Altered
		ItemSystem.GiveItemPack(actor.id, "Player_Default", false, true);
	end

	if (actor.actor and not actor.actor:IsMyPlayer()) then -- EI Altered
...
function SinglePlayer:OnNewUsable(srcId, objId, usableId)
	...
	if self.UsableMessage then
		-- EI HUD.SetInstructionObsolete(self.UsableMessage)
...
-- EI	if(UIAction) then
-- EI		UIAction.StartAction("DisplayUseText", {msg}); --this triggers the UIAction "DisplayUseText" and pass the msg as argument (see FlowGraph UIActions how to send msg to flash)
-- EI	end
end
...
function SinglePlayer:ProcessActorDamage(hit)
	...
	--update the health		
	local newhealth = health - totalDamage; -- EI we want fractions for health and not whole numbers. old code: = math.floor(health - totalDamage);
...
function SinglePlayer:AreActionable(src, objs)
	if (objs) then
		for i,entity in ipairs(objs) do
			actionableEntityList[i] = entity:IsActionable(src);
		end
	end

	return actionableEntityList;
end

function SinglePlayer:IsActionable(srcId, objId)
	if not objId then return 0 end;

	local obj = System.GetEntity(objId);
	if (obj.IsActionable) then
		if (obj:IsHidden()) then
			return 0;
		end;
		local src = System.GetEntity(srcId);
...
function SinglePlayer:DidActionsChange(objId)
	local obj = System.GetEntity(objId);
	if (obj.DidActionsChange) then
		return obj:DidActionsChange();
	end
	return 0;
end

function SinglePlayer:OnNewActionable(srcId, objId, usableId)
	if not srcId then return end
	if objId and not System.GetEntity(objId) then objId = nil end
...
function SinglePlayer:GetActions(srcId, objId)
	if not objId then return 0 end;
...
function SinglePlayer:PerformAction(srcId, objId, name)
	if not objId then return 0 end;
...
function SinglePlayer:DisplayActionableMenu(srcId, objId, index)
	if srcId ~= g_localActorId then return end
