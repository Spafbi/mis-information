...
NPCVendor =
{
	Properties =
	{
		...
		-- must set each of these when the vendor is added to the map
		Vendor =
		{
			VendorId = 0, -- must be greater than 0
			LootItemCategory = "",
		},
	},
...
MakeDerivedEntityOverride( NPCVendor,BasicEntity )

function NPCVendor:LoadModelOnDemand()
	return self.Properties.Cinematic.bOnDemandModelLoad;
end

------------------------------------------------------------------------------------------------------
function NPCVendor:SetFromProperties()
...
------------------------------------------------------------------------------------------------------
function NPCVendor:SetupModel()
	if (self:LoadModelOnDemand()==0 or System.IsEditor()) then
		self:LoadAndPhysicalizeModel();
	else
		Game.CacheResource("NPCVendor.lua", self.Properties.object_Model, eGameCacheResourceType_StaticObject, 0);
	end
end

-------------------------------------------------------------------------------------------------------
function NPCVendor:LoadAndPhysicalizeModel()
...
-------------------------------------------------------------------------------------------------------
function NPCVendor:UnloadModel()
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_ResetAnimation()
	self.controllingAnimHere = true;
	self:ResetAnimation(0, -1);
	self.animstarted=0;
	--
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_StartAnimation(sender)
	self.controllingAnimHere = true;
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_StopAnimation(sender)
	self.controllingAnimHere = true;
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_RagdollizeDerived()
	self.isRagdollized = true;
end

------------------------------------------------------------------------------------------------------
function NPCVendor:Event_ModelLoad()
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_ModelUnload()
...
------------------------------------------------------------------------------------------------------
function NPCVendor:Event_RenderAlwaysEnable()
	self:RenderAlways(1);
end

------------------------------------------------------------------------------------------------------
function NPCVendor:Event_RenderAlwaysDisable()
	self:RenderAlways(0);
end

------------------------------------------------------------------------------------------------------
function NPCVendor:DoStartAnimation()
	self.animName = self.Properties.Animation.Animation;
...
------------------------------------------------------------------------------------------------------
function NPCVendor:StartEntityAnimation()
...
------------------------------------------------------------------------------------------------------
function NPCVendor.Client:OnTimer(timerId,mSec)
...
------------------------------------------------------------------------------------------------------
function NPCVendor:CorrectTiming(frameTime)
...
------------------------------------------------------------------------------------------------------
function NPCVendor.Server:OnUpdate(dt)
...
------------------------------------------------------------------------------------------------------
function NPCVendor.Client:OnUpdate(dt)
...
-------------------------------------------------------
function NPCVendor:OnLoad(table)
...
-------------------------------------------------------
function NPCVendor:OnSave(table)
...
------------------------------------------------------------------------------------------------------
NPCVendor.FlowEvents.Inputs.ResetAnimation = { NPCVendor.Event_ResetAnimation, "bool" }
NPCVendor.FlowEvents.Inputs.StartAnimation = { NPCVendor.Event_StartAnimation, "bool" }
NPCVendor.FlowEvents.Inputs.StopAnimation = { NPCVendor.Event_StopAnimation, "bool" }

NPCVendor.FlowEvents.Inputs.ModelLoad = { NPCVendor.Event_ModelLoad, "bool" }
NPCVendor.FlowEvents.Inputs.ModelUnload = { NPCVendor.Event_ModelUnload, "bool" }


-- client functions
function NPCVendor.Client:ClEvent_StartAnimation(servertime)
...
function NPCVendor.Client:ClEvent_ResetAnimation()
	if( not CryAction.IsServer() ) then
...
------------------------------------------------------------------------------------------------------
function NPCVendor:PhysicalizeThis()
...
------------------------------------------------------------------------------------------------------
function NPCVendor:SendSyncToClient( channelId )
...
function NPCVendor.Server:OnPostInitClient( channelId )
	self:SendSyncToClient(channelId);
end


function NPCVendor.Server:SVSync(channelId)
	self:SendSyncToClient(channelId);
end


function NPCVendor.Client:ClSync( animTimeValue, startTimeValue, serverTimeValue )
...
function NPCVendor:UpdateFromServer()
	self.server:SVSync();
end


-- notifications from PlaySequence FG node (entire sequence starts/stops)
function NPCVendor:OnSequenceStart()
	self.sequenceStatus = SEQUENCE_PLAYING;
end


function NPCVendor:OnSequenceStop()
	self.sequenceStatus = SEQUENCE_STOPPED;
end


-- Notifications from trackview (animation in sequence starts/stops)
function NPCVendor:OnSequenceAnimationStart( animName )
	self.sequenceStatus = SEQUENCE_PLAYING;
	self.animName = animName;
end


function NPCVendor:OnSequenceAnimationStop()
	self.sequenceStatus = SEQUENCE_STOPPED;
end


function NPCVendor:OnFlowGraphAnimationStart( animName, layer, loop )
	self.animName = animName;
...
function NPCVendor:OnFlowGraphAnimationStop()
...
function NPCVendor:OnFlowGraphAnimationEnd()
...
function NPCVendor:IsActionable(user)
	--Log("[ NPCVendor ] IsActionable");

	return 1;
end

function NPCVendor:GetActions(user)
	--Log("[ NPCVendor ] GetActions");

	local actions = {};
	
	actions[#actions+1] = "@item_open_inventory"

	return actions;
end

function NPCVendor:PerformAction(user, action)
	--Log("[ NPCVendor ] PerformAction: action="..action);

	-- send back up to the gamerules to perform
