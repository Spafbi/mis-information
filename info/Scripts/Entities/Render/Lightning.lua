...
function Lightning:LoadLightToSlot( nSlot )
	...
	-- EI Hendrik, reuse Light, avoid LoadLight calls for updates
	if self:IsSlotLight(nSlot) then
		self:SetLightColorParams( nSlot, { x=clr, y=clr, z=clr }, clr );
	else
...
function Lightning:Event_Strike()
	if (self.bStriking == 0) then
		...
		self.vStrikeOffset.z = math.random()*10 -- EI Hendrik Add a bit random height
		...
		if (props.bRelativeToPlayer == 1) then
			...
			self.vStrikeOffset.z = self.vStrikeOffset.z + (vCamPos.z - vEntityPos.z); -- EI Hendrik Relative offset required
		end
		...
		self.vSkyHighlightPos.z = vEntityPos.z + self.vStrikeOffset.z; -- EI Hendrik also include Strike vertical offset

		self.vStrikeOffset.z = self.vStrikeOffset.z + Effects.SkyHighlightVerticalOffset*0.9; -- EI Hendrik Strike vertical relative offset required
