...
Door =
{
	Properties =
	{
		...
		fileModel = "Objects/props/Doors/FlatWhite_Door.cgf",
		Audio =
		{
			audioTriggerOnMoveOpenTrigger = "Play_door_wood_open",
			audioTriggerOnMoveCloseTrigger = "Play_door_wood_close",
			audioTriggerOnStopTrigger = "",
			audioTriggerOnClosedTrigger = "Play_door_wood_closed",
			eiSoundObstructionType = 0, -- Anything greater than 1 will be reset to 2.
		},
		Rotation =
		{
			fSpeed = 200.0,
...
function Door:IsActionable(user)
...
function Door:GetActions(user)
	...
	actions[i] = "@use_door"; i=i+1;

	return actions;
end

function Door:PerformAction(user, action)
	--Log("[ Door ] PerformAction: action="..action);
