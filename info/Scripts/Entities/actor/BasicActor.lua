...
BasicActorParams =
{
	...
	gameParams =
	{
		...
		sprintMultiplier = 1.5, -- Speed is multiplied by this amount if sprint key is pressed
		crouchMultiplier = 1.0, -- Speed is multiplied by this amount if crouching
		strafeMultiplier = 1.0, -- Speed is multiplied by this amount when strafing
		backwardMultiplier = 0.7, -- Speed is multiplied by this amount when going backward
		jumpHeight = 1.0, -- In meters - EI changed from 1.5 originally
		...
		sBodyFireParticleEffectPath = "fire.flame.player_burning",
		bodyFireParticleEffectSlot = 5,
	},
}
...
function BasicActor.Server:OnHit(hit)
	...
	if(not isMultiplayer) then
		...
		-- Check if the weapon is silenced
		local isSilenced = false; --self:HasSilencedWeapon(hit.weaponId);
	...
	elseif (event == "jump_splash") then
		...
		if(not str) then
			...
			if(self.actor:IsLocalClient()) then
				--local sound = self:PlaySoundEventEx("sounds/physics:footsteps/player:water_shallow_land_fp", SOUND_DEFAULT_3D, 0, volume, g_Vectors.v000, -1, -1, SOUND_SEMANTIC_PLAYER_FOLEY);
			end
		end
